...
def create_weathercond_df(df):
    weathercond_df = df.groupby(by=[df["datetime"].dt.year.astype(str),df["season"]]).agg({
            "total": "sum"
            }).reset_index()
    return weathercond_df
    
def create_weathercond_year_df(df):
    weathercond_year_df = df.groupby(by=[df["datetime"].dt.year.astype(str),df["weather"]]).agg({
            "total": "sum"
            }).reset_index()
    return weathercond_year_df

# import dataset
bike_df = pd.read_csv('hour_new.csv')
